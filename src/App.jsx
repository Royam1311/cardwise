  ...
  return (
    <main className="cinematic-auth">
      <div className="cinematic-stage">
        <img
          className="cinematic-frame cinematic-frame--start"
          src="/benefy-login-start.webp"
          alt=""
          aria-hidden="true"
        />
        <img
          className="cinematic-frame cinematic-frame--end"
          src="/benefy-login-end.webp"
          alt=""
          aria-hidden="true"
        />

        <div className="cinematic-screen">
          <section className="cinematic-login-content" aria-label={mode === 'login' ? t.signIn : t.createAccount}>
            <AdaptiveLogo className="cinematic-auth-logo" />
            <h1 className="cinematic-login-title">{mode === 'login' ? t.signIn : t.createAccount}</h1>
            <p className="cinematic-login-subtitle">{t.loginText}</p>

            <button type="button" className="cinematic-google-button" onClick={google} disabled={googleBusy || busy}>
              <GoogleMark />
              <span>{googleBusy ? t.googleLoading : t.google}</span>
            </button>

            <div className="cinematic-divider"><span>{t.or}</span></div>

            <form className="cinematic-login-form" onSubmit={submit}>
              ...
              <label className="cinematic-field">
                ...
                <input type="password" minLength="6" value={password} onChange={event => setPassword(event.target.value)} required autoComplete={mode === 'login' ? 'current-password' : 'new-password'} />
              </label>
              <button type="submit" className="cinematic-submit-button" disabled={busy || googleBusy}>
                {busy ? t.loading : mode === 'login' ? t.login : t.register}
              </button>
            </form>

            {msg && <div className="cinematic-message" role="status">{msg}</div>}

            <div className="cinematic-login-actions">
              <button type="button" className="cinematic-text-button" onClick={switchMode}>
                {mode === 'login' ? t.noAccount : t.haveAccount}
              </button>
              <button type="button" className="cinematic-demo-button" onClick={onDemo}>{t.demo}</button>
            </div>
          </section>
        </div>
      </div>
...
function WalletCard({ code, name, selected, onToggle, t }) {
  ...
  return (
    <button type="button" className={`wallet-card wallet-card--${theme} ${selected ? 'is-selected' : ''}`} onClick={onToggle}>
